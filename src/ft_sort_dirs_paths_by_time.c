...
/*   Updated: 2026/06/08 22:22:47 by tel-bouh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_sort_array_paths_by_time(t_data *data)
{
	...
	while (i + 1 < data->paths.nbr_of_paths)
	{
		...
		{
			if (ft_cmp_time_dir_path(data->paths.raw_time[i],
					data->paths.raw_time[i + 1]))
...
int	ft_sort_dirs_paths_by_time(t_data *data)
{
	...
	i = 0;
	data->paths.raw_time = malloc(sizeof(time_t)
